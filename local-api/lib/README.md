# formlabs_local_api
© 2024 Formlabs - All Rights Reserved

# Introduction
The Formlabs Local API is designed for integrations that want to automate
job preparation, getting local-network printer status, or send jobs to printers
...
the reference docs. Eiger's API is described by an [OpenAPI Specification](https://spec.openapis.org/oas/v3.1.0).
...
application to expose the HTTP API and then making HTTP API calls in your own code.
This application is like Formlab's regular PreForm job preparation application,
...
downloads for each Operating System. There is also experimental support for Linux
for running within a Docker container.
...
- **Scene**: The current state of a job that can be printed on a particular printer model.
  This includes both the “Scene Type” and all of the currently loaded models and their support structures.
...
Requests made asynchronous will use the last cached state. For example, if a “get scene”
